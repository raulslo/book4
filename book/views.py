...
def book_genre_detective(request):
    book = models.Book.objects.filter(genre='DETECTIVE').order_by("-id")
    return render(request, 'book_list.html', {'book': book})


def book_genre_romace(request):
    book = models.Book.objects.filter(genre='ROMACE').order_by('-id')
    return render(request, 'book_list.html', {'book': book})


def book_genre_fantasies(request):
    book = models.Book.objects.filter(genre='FANTASIRS').order_by('-id')
    return render(request, 'book_list.html', {'book': book})


def book_genre_anime(request):
    book = models.Book.objects.filter(genre='ANIME').order_by('-id')
    return render(request, 'book_list.html', {'book': book})
